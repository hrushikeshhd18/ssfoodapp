...
class PickMapScreen extends StatelessWidget {
          ...
          return Stack(children: [
            ...
            GoogleMap(
              initialCameraPosition: CameraPosition(
                target: fromAddAddress ? LatLng(locationController.position.latitude, locationController.position.longitude)
                    : _initialPosition,
                zoom: 17,
              ),
            ...
            Positioned(
              ...
              child: !locationController.isLoading ? CustomButton(
                buttonText: locationController.inZone ? fromAddAddress ? 'pick_address'.tr : 'pick_location'.tr
                    : 'service_not_available_in_this_area'.tr,
                onPressed: (locationController.buttonDisabled || locationController.loading) ? null : () {
                  if(locationController.pickPosition.latitude != 0 && locationController.pickPlaceMark.name != null) {
                    if(fromAddAddress) {
                      if(googleMapController != null) {
                        googleMapController.moveCamera(CameraUpdate.newCameraPosition(CameraPosition(target: LatLng(
                      ...
                      locationController.saveAddressAndNavigate(_address, fromSignUp, route, canRoute);
                    }
                  }else {
